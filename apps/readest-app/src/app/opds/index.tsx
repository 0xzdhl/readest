...
const opdsSearchSchema = z.object({
...
export const Route = createFileRoute('/opds/')({
  validateSearch: opdsSearchSchema,
  component: OPDSBrowserPage,
});
...
function OPDSBrowserPage() {
  ...
  const catalogUrl = searchParams.url || '';
  const catalogId = searchParams.id || '';
      ...
      try {
        ...
        if (!res.ok) {
          if (isSearch && res.status === 404) {
            ...
            setTimeout(() => {
              // router.history.back() (not closeOPDSBrowser) so the user can
