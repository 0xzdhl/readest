...
import { BACKUP_DIALOG_EVENT } from './components/backupDialog';

const BackupWindow = React.lazy(async () => {
  const module = await import('./components/BackupWindow');
  return { default: module.BackupWindow };
});

const libraryDefaultSearch = {
  group: '',
  groupBy: '',
  opds: '',
};
...
const librarySearchSchema = z.object({
  group: z.string().default(libraryDefaultSearch.group).catch(libraryDefaultSearch.group),
  groupBy: z.string().default(libraryDefaultSearch.groupBy).catch(libraryDefaultSearch.groupBy),
  opds: z.string().default(libraryDefaultSearch.opds).catch(libraryDefaultSearch.opds),
});

export const Route = createFileRoute('/library/')({
  validateSearch: librarySearchSchema,
  search: {
    middlewares: [stripSearchParams(libraryDefaultSearch)],
  },
...
const LibraryPageContent = () => {
  ...
  const [showCatalogManager, setShowCatalogManager] = useState(searchParams.opds === 'true');
  const [shouldRenderBackupWindow, setShouldRenderBackupWindow] = useState(false);
  ...
  useEffect(() => {
    sessionStorage.setItem('lastLibraryParams', window.location.search.slice(1) || '');
  }, [searchParams]);

  useEffect(() => {
    const handleBackupDialogEvent = (event: Event) => {
      const customEvent = event as CustomEvent<{ visible?: boolean }>;
      if (customEvent.detail?.visible) {
        setShouldRenderBackupWindow(true);
      }
    };

    window.addEventListener(BACKUP_DIALOG_EVENT, handleBackupDialogEvent);
    return () => {
      window.removeEventListener(BACKUP_DIALOG_EVENT, handleBackupDialogEvent);
    };
  }, []);

  // Unified navigation function that handles scroll position and direction.
  // Keep `group` explicitly present even when it is empty so the validated
  // search shape stays stable during TanStack Router navigation.
  const handleLibraryNavigation = useCallback(
    (targetGroup: string) => {
      const currentGroup = searchParams.group || '';
      ...
      // even when targetGroup is '' so empty-group drops stay stable.
  ...
  useEffect(() => {
    ...
    const groupByParam = searchParams.groupBy;
  ...
  return (
    ...
    >
      ...
      <UpdaterWindow />
      <MigrateDataWindow />
      {shouldRenderBackupWindow && (
        <Suspense fallback={null}>
          <BackupWindow onPullLibrary={pullLibrary} initialVisible />
        </Suspense>
      )}
