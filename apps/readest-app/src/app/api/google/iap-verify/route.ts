...
/**
 * Owner-only IAP verification (Google sibling of apple/iap-verify).
 * `rlsMiddleware` opens the RLS-scoped tx and resolves the session; the
 * handler parses, verifies with Google, then writes through `tx`.
 */
