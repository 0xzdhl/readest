...
/**
 * GET /api/storage/list — owner-only, paginated. The fan-out "include every
 * sibling file in the matched groups" pass is preserved so library UIs see
 * complete book/replica groups even when a search filter only matched a
 * subset.
 */
    ...
    handlers: {
      GET: async ({ request, context }) => {
        ...
        try {
          ...
          // Whitelist mirrors the legacy supabase route's allow-list verbatim
          // so a malformed sortBy param falls back to `created_at` rather
          // than erroring.
