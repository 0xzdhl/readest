...
export const renderShareOgImage = async (
	...
	const result = await resolveActiveShare(token, tx);
	if (!result.ok) {
...
const withCoverCard = (cover: string, title: string, author: string | null) => (
	...
	>
		...
		>
			...
			{author && (
				<div style={{ fontSize: 32, color: "#525252", fontWeight: 400 }}>
					{clamp(author, 60)}
				</div>
			)}
			<div style={{ flex: 1 }} />
			<div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
				<div style={{ fontSize: 22, color: "#0066cc", fontWeight: 500 }}>
					Shared via Readest
				</div>
...
const textOnlyCard = (title: string, author: string | null) => (
	...
	>
		<div style={{ display: "flex", flexDirection: "column", gap: "32px" }}>
			...
			{author && (
				<div style={{ fontSize: 40, color: "#525252", fontWeight: 400 }}>
					{clamp(author, 60)}
				</div>
			)}
		</div>
		<div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
			<div style={{ fontSize: 26, color: "#0066cc", fontWeight: 500 }}>
				Shared via Readest
			</div>
