    ...
    handlers: {
      POST: async ({ request, params }) => {
        ...
        return runProtected(request, async ({ user, tx }) => {
          ...
          // RLS already gates `user_id = current_setting('app.user_id')`,
          // so a wrong-user lookup just returns 0 rows (404). Keep the
          // explicit owner check anyway so the audit trail in the
          // response distinguishes "no such share" from "not yours".
          ...
          if (share.userId !== user.id) {
            ...
            // belt-and-braces clarity if a future refactor ever loosens
            // the policy.
