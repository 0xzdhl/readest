  ...
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        ...
        // INCLUDING soft-deleted ones. file_key is unique globally, so an
        // active import that the user later deleted leaves a row that would
        // collide with a fresh insert below — we restore it instead of failing.
        ...
        if (existingError) {
          ...
          return Response.json({ error: 'Could not check library' }, { status: 500 });
        }
        const existingRows = (existing ?? []).filter(
          (f) => !/\.(png|jpe?g|webp|gif)$/i.test(f.file_key),
        );
        ...
        if (deletedRow) {
          // Restore the soft-deleted row so the unique file_key constraint
          // isn't hit by a fresh insert. The bytes may also still be in
          // storage; if the copy below succeeds it overwrites them, if it
          // doesn't we leave the row in its restored state so the user can
          // re-attempt later.
        ...
        // Quota check before doing any byte-copy work. JWT-based but
        // consistent with how the existing upload endpoint enforces it.
        ...
        // bytes, then mark the row clean. On copy failure we soft-delete the
        // row so the user's library doesn't show a phantom book.
