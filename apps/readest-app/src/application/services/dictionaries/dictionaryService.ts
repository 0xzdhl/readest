...
import type { ImportedDictionary, ImportDictionariesResult } from '@/domain/dictionaries';
...
const importMdictBundle = (
  ...
  Effect.gen(function* () {
    ...
    // `Effect.all` defaults to sequential; the legacy code used `Promise.all`,
    // so request unbounded concurrency to match the original read parallelism.
    const mddFiles = yield* Effect.all(
      group.mdd.map((m) => readSource(m.source)),
      { concurrency: 'unbounded' },
    );
    const cssFiles = yield* Effect.all(
      group.css.map((c) => readSource(c.source)),
      { concurrency: 'unbounded' },
    );
