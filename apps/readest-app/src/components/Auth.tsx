...
import { authWithCustomTab, authWithSafari, storeBearerFromCallback } from "@/app/auth/utils/nativeAuth";
...
export function AuthComponent() {
		...
		try {
			if (mode === "signin") {
				...
				const { error } = await authClient.requestPasswordReset({
					email,
					redirectTo: isTauriAppPlatform()
						? DEEPLINK_CALLBACK
						: `${getBaseUrl()}/auth/recovery`,
				});
				if (error)
					setErrorMsg(error.message ?? _("Failed to send reset email"));
	...
	const handleMagicLink = async () => {
		...
		try {
			...
			setErrorMsg(
				err instanceof Error ? err.message : _("Failed to send magic link"),
			);
		} finally {
			setLoading(false);
		}
	...
	useEffect(() => {
		...
		invoke("get_environment_variable", { name: "USE_CUSTOM_OAUTH" }).then(
			(value) => {
				if (value === "true") {
	...
	const renderEmailForm = () => (
		<form onSubmit={handleEmailSubmit} className="flex w-64 flex-col gap-2">
			...
			{mode !== "forgot" && (
				<>
					<label
						...
						className="text-base-content/75 mt-1 text-xs"
					>
			...
			{statusMsg && (
				<div className="text-base-content/75 text-xs">{statusMsg}</div>
			)}
		...
		>
			...
			>
				...
				>
					...
					<hr
						aria-hidden="true"
						className="border-base-300 my-3 mt-6 w-64 border-t"
					/>
					{renderEmailForm()}
				</div>
			</div>
