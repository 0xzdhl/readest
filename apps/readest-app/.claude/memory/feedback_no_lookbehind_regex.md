...
**Why:** The project has a `check:lookbehind-regex` build check (`pnpm check:all`) that scans the Vite build output chunks and fails if any lookbehind assertions are found. Older WebViews (especially on some Android devices) don't support lookbehinds.
